...
import { decentra_verify_m_backend } from 'declarations/decentra_verify_m_backend';
...
function Block() {
  ...
  const showPublicKeySchnorr = async () => {
    try {
      setLoading(true);
      setError(undefined);
      const key = await decentra_verify_m_backend.get_schnorr_public_key();
      setKey(key);
    } catch (err) {
      console.error(err);
      setError(String(err));
  ...
  const signMessage = async () => {
    try {
      setLoading2(true);
      setError(undefined);
      const signature = await decentra_verify_m_backend.sign_message_with_schnorr(message);
  ...
  return (
    <div className="space-y-4">
      <h1 className="mb-6 text-center text-2xl font-bold text-gray-800">Schnorr Signing</h1>
      ...
      {signature && (
        <div className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
          <h3 className="mb-2 text-sm font-medium text-gray-800">Schnorr signature:</h3>
